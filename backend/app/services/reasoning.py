...
async def detect_findings(db, ws: str) -> int:
    ...
    for c in commitments:
        ...
        desired.append({
            "dedupe_key": f"untracked:{c.id}", "kind": "gap",
            "title": f"Promised but not tracked: {_clip(c.name, 80)}",
            "detail": detail,
            "entity_ids": [c.id], "artifact_ids": [src.id] if src else [],
    ...
    for c in commitments:
        ...
        done = next((a for a in linear_arts
                     if (a.meta or {}).get("stateType") == "completed"
                     and (a.meta or {}).get("identifier") == ident), None)
        if not done:
            continue
        if c.state != "delivered":
            ...
            c.updated_at = _now()
        desired.append({
            "dedupe_key": f"win:{c.id}", "kind": "win",
            ...
            "entity_ids": [c.id], "artifact_ids": [done.id], "action": None,
        })
    ...
    if len(unlinked) >= _UNREQUESTED_THRESHOLD:
        ...
        desired.append({
            "dedupe_key": "unrequested-work", "kind": "drift",
    ...
    open_count = (await db.execute(select(Insight).where(
        ...
        Insight.status == "open"))).scalars().all()
...
async def _upsert(db, ws: str, desired: list[dict], corrections: str = "") -> None:
    ...
    for d in desired:
        ...
        if cur:
            ...
            db.add(Insight(
                id=f"in_{uuid.uuid4().hex[:10]}", workspace_id=ws, origin="model",
                kind=d["kind"], title=d["title"][:200], detail=d["detail"],
                entity_ids=d["entity_ids"], artifact_ids=d["artifact_ids"], action=action,
                dedupe_key=d["dedupe_key"], evidence={}, status="open", created_at=_now(),
            ))
...
def _fallback_brief(open_findings: list[Insight]) -> dict:
    ...
    headline = (f"{len(open_findings)} things need attention" if open_findings
                else "Everything tracked is on course")
    summary = (f"{gaps} commitment(s) have no tracked work, "
...
async def generate_brief(db, ws: str) -> Insight:
    ...
    open_findings = (await db.execute(select(Insight).where(
        Insight.workspace_id == ws, Insight.origin == "model",
        Insight.status == "open").order_by(Insight.created_at.desc()))).scalars().all()
    ...
    brief = (await db.execute(select(Insight).where(
        Insight.workspace_id == ws, Insight.origin == "model",
        Insight.dedupe_key == "brief"))).scalars().first()
    if not brief:
        brief = Insight(id=f"in_{uuid.uuid4().hex[:10]}", workspace_id=ws, origin="model",
                        kind="brief", dedupe_key="brief", status="open", created_at=_now())
        db.add(brief)
