...
SYSTEM_PROMPTS: dict[str, str] = {
    ...
    "orbit-agent": (
        ...
        "- Decide when to pull fresh data instead of trusting memory: if the question asks about the "
        "CURRENT state ('right now', 'today', 'latest', 'status of X') and memory's newest matching "
        "item looks older than the question implies — or memory has nothing while a relevant tool is "
        "connected — call pull_connector for THAT source, then search_memory again. Historical and "
        "analytical questions ('what did we promise', 'trends this quarter') are answered from memory; "
        "pulling adds nothing. Pull a source at most ONCE per question; if the pull changes nothing, "
        "answer from what exists and say what's missing.\n"
