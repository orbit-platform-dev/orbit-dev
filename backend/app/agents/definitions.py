...
SYSTEM_PROMPTS: dict[str, str] = {
    "extractor": (
        ...
        "- commitments: only explicit promises of WORK someone made ('we will X') — a deliverable, "
        "fix, feature, check or deadline someone could track to done. text = the promise in plain "
        "words; to = the customer/person it was promised to, exactly as named; due = the deadline as "
        "stated ('before Oct renewal'), empty if none. Discussion or ideas are NOT commitments, and "
        "personal or emotional promises ('I will always support you') are NEVER commitments.\n"
    ...
    "orbit-agent": (
        ...
        "- duckduckgo_search reaches the public web — use it ONLY for world facts the company's own "
        "memory cannot contain (a library, a competitor, an API, a news event), and say the answer came "
        "from the web. Company questions are NEVER answered from the web.\n"
        ...
        "DETAIL questions ('what changed in this PR', 'why was this rejected', 'what did the review "
        "say') need the FULL item: search first, then read_item on the id you found — search snippets "
        "are truncated and cut off diffs. When you explain a code change, quote the relevant lines in a "
        "fenced code block (```diff) and name the files; never describe a diff you did not read.\n\n"
