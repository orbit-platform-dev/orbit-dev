  ...
  if (k === "slack") {
    return (
      <div className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-border bg-white", className)}>
        <svg role="img" aria-label="Slack" viewBox="0 0 512 512" className="h-1/2 w-1/2" xmlns="http://www.w3.org/2000/svg">
          {SLACK_MARK.map((p, i) => <path key={i} d={p.d} fill={p.fill} />)}
        </svg>
      </div>
    );
  }
